...
export async function sendLeadNotification(lead: Lead) {
  ...
  const subject = `Nuevo Lead Elite - ${lead.full_name} (${lead.plan_selected})`;
  ...
  const emailHtml = `
    <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 24px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
      <div style="background-color: #2563eb; padding: 32px; text-align: center;">
        <h1 style="color: white; font-size: 24px; margin: 0; text-transform: uppercase; letter-spacing: 0.1em; font-weight: 900;">Elite Lead Lab</h1>
      ...
      <div style="padding: 24px; background-color: #f8fafc; text-align: center; border-top: 1px solid #f1f5f9;">
        <p style="color: #94a3b8; font-size: 11px; margin: 0;">Ancastav Digital Services Platform v3.0 | Elite Infrastructure</p>
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: 'Elite Lab <notificaciones@ancastav.com>',
      to: recipients,
      subject: subject,
      html: emailHtml,
    });
